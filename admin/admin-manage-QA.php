<?php

include 'admin-verify-loggedin.php';
include 'db-connect.php';

if($_SERVER['REQUEST_METHOD'] === 'POST') {
  $question_id = $_POST['question_id'];
  $question = $_POST['question'];
  $answer = $_POST['answer'];

  if($stmt = $conn->prepare("INSERT INTO question (question, answer) VALUES (?,?)")) {
    if($stmt->bind_param("ss",$question, $answer)) {
      if($stmt->execute()) {
        $del_stmt = $conn->prepare("DELETE FROM unanswered_question WHERE question_id=?");
        $del_stmt->bind_param("i", $question_id);
        $del_stmt->execute();
        $del_stmt->close();
      }
    }
    $stmt->close(); 
  } 
}

$stmt = $conn->prepare("SELECT * FROM unanswered_question");
$stmt->bind_param();
$stmt->execute();
$result = $stmt->get_result();

$unanswered_questions = "";
while($row = $result->fetch_assoc()) {
  $unanswered_question_id = $row['question_id'];
  $unanswered_question = $row['question_text'];
  $unanswered_questions = $unanswered_questions . 
   '<div class="w3-container w3-card w3-white w3-round w3-margin "><br>
      <form id="admin-add-QA-form'.$unanswered_question_id.'" action="admin-manage-QA.php" method="post">
        <p>'.$unanswered_question.'</p>
        <input type="hidden" name="question_id" value="'.$unanswered_question_id.'">
        <input type="hidden" name="question" value="'.$unanswered_question.'">
        <textarea form_id="admin-add-QA-form'.$unanswered_question_id.'" class="w3-border w3-padding w3-block" name="answer"></textarea>
        <button type="button" class="w3-button w3-theme-d1 w3-margin-bottom w3-margin-top" onclick="submitForm(this)"> &nbsp;Answer</button> 
      </form>
    </div>';
}

$stmt->close();
$conn->close();
?>

<!DOCTYPE html>
<html>

<head>
  <?php include 'admin-head.php'; ?>
</head>

<body class="w3-theme-l5">

<!-- Navbar -->
<?php include "admin-navbar.php"; ?>

<!-- Page Container -->
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">    
  <!-- The Grid -->
  <div class="w3-row">
    <!-- Left Column -->
    <div class="w3-col m3">
      <?php include "admin-sidepanel.php"; ?>          
    <!-- End Left Column -->
    </div>
    
    <!-- Right Column -->
    <div class="w3-col m9">
    
      <div class="w3-row-padding">
        <div class="w3-col m12">
          <div class="w3-card w3-round w3-white">
            <div class="w3-container w3-padding">
              <h6 class="w3-opacity">Enter the question and answer</h6>
              <form id="admin-manage-QA-form" action="admin-manage-QA.php" method="post">
                <label><b>Question</b></label>
                <input class="w3-border w3-padding w3-block w3-margin-bottom" type="text" name="question">
                <label><b>Answer</b></label>
                <textarea form_id="admin-manage-QA-form" row="6" class="w3-border w3-padding w3-block" name="answer"></textarea>
                <button type="button" class="w3-button w3-theme w3-margin-top w3-margin-bottom" onclick="submitForm(this)"><i class="fa fa-plus"></i> &nbsp;Add</button>
              </form>
            </div>
          </div>
        </div>
      </div>
      
      <?php echo $unanswered_questions; ?>
    <!-- End Right Column -->
    </div>
        
  <!-- End Grid -->
  </div>
  
<!-- End Page Container -->
</div>
<br>

<!-- Footer -->
<?php include "admin-footer.php"; ?>
 
<script>
  // Used to submit closest form
function submitForm(input) {
  $(input).closest("form").submit();
}

// Used to display image on selecting
function readURL(input) {
  if (input.files && input.files[0]) {
      var reader = new FileReader();

      reader.onload = function (e) {
          $("#profile-pic")
              .attr("src", e.target.result)
      };

      reader.readAsDataURL(input.files[0]);
  }
}
</script>

</body>
</html>

