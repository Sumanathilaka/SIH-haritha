...
$_SESSION['admin_add_volunteer_status'] = "FAILED";
...
if (count($_FILES) > 0) {
    if ($_FILES['profile_pic']['tmp_name'] != NULL) {
    	$upload_filename = basename($_FILES['profile_pic']['tmp_name']);
		$upload_file_path = $uploaddir . $upload_filename;
...
if(!$verify_response['status']){
	$_SESSION['admin_add_volunteer_error'] = $verify_response['error_message'];
...
if($stmt->num_rows != 0){
	$_SESSION['admin_add_volunteer_error'] = "Username already exists";
	header("Location: admin-manage-volunteers.php");
	exit;
}
$stmt->close();
...
$stmt2 = $conn->prepare("INSERT INTO userdetails (username, name, email, mobile, address, latitude, longitude, postalcode, coordinator, image_id) VALUES (?,?,?,?,?,?,?,?,?,?)");

if($stmt1 and $stmt2) {
	if($stmt1->bind_param("sss", $username, $password, $role) and $stmt2->bind_param("sssisddiss", $username, $name, $email, $telephone, $address, $latitude, $longitude, $postalcode, $coordinator, $upload_filename)) {
		if($stmt1->execute() and $stmt2->execute()) {
			$_SESSION['admin_add_volunteer_status'] = "SUCCESS";
		}
	}
}
